const users=require("../model/categories");
// post user deails
exports.insertCategoriesInfo = async(req,res)=>{
    var check =await users.selectName(req.body.name);

        if (check && check.rowCount===0){
            var query =await users.insertCategories(req.body);
...
//get all details 
exports.allCategoriesId = async(req,res)=>{
