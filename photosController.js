const users=require("../model/photos");
// post user deails
exports.insertPhotoInfo = async(req,res)=>{
    var check =await users.selectName(req.body.collection_id);

        if (check && check.rowCount===0){
            var query =await users.insertPhoto(req.body);
            var result = await users.selectName(req.body.collection_id);
...
//get all details 
exports.allPhotoId = async(req,res)=>{
  ...
  exports.deleteusers= async (req, res) => {
    ...
    exports.updatePhoto=async(req,res)=>{
      ...
      if(check && check.rowCount===0){
          ...
          var query= await users.updatePhoto(req.body);
          console.log(query);
