...
// post user deails
exports.insertAdminInfo = async(req,res)=>{
    var check =await users.selectName(req.body.name);

        if (check && check.rowCount===0){
            var query =await users.insertAdmin(req.body);
...
//get all details 
exports.allAdminId = async(req,res)=>{
  ...
  exports.deleteusers= async (req, res) => {
    ...
    exports.updateAdmin=async(req,res)=>{
      ...
      if(check && check.rowCount===0){
          ...
          var query= await users.updateAdmin(req.body);
          console.log(query);
