const users=require("../model/studios");
// post user deails
exports.insertStudioInfo = async(req,res)=>{
    var check =await users.selectName(req.body.name);

        if (check && check.rowCount===0){
            var query =await users.insertStudio(req.body);
...
//get all details 
exports.allStudioId = async(req,res)=>{
  ...
  exports.deleteusers= async (req, res) => {
    ...
    exports.updateStudio=async(req,res)=>{
      var check=await users.selectName(req.body.location);
  
      if(check && check.rowCount===0){
          res.json({status:"FALSE",message:"user not present"});
      }
      else{
          var query= await users.updateStudio(req.body);
          console.log(query);
